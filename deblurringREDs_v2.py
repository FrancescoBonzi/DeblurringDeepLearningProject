...
'''
    MEMORY OPTIMIZATION.
The dataset is loaded in its splitted version, avoiding keep in memory
            ...
            for j, frame in enumerate(frames):
                ...
                for w in range(num_patches_width):
                    ...
                    for h in range(num_patches_height):
                        ...
                        loaded_dataset[i*patches+w*num_patches_height+h, :, :, :] = img[start_heigth:(
                            start_heigth+height+2*num_conv), start_width:(start_width+width+2*num_conv), :]
    return loaded_dataset

def restore_images(splitted_images):
    restored_images = np.zeros(int(len(splitted_images)/patches),
                           original_height, original_width, 3)
    for i in range(int(len(splitted_images)/patches)):
        for w in range(num_patches_width):
            start_width = get_left_overlap(w, num_patches_width)
            for h in range(num_patches_height):
                start_height = get_left_overlap(h, num_patches_height)
                restored_images[i, w*width:(w+1)*width, h*height:(h+1)*height, :] = splitted_images[i*patches + w*num_patches_height+h, start_height:start_height+height, start_width:start_width+width, :]
